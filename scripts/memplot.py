# ComfyUI_DWposeDeluxe/scripts/memplot.py
...
def create_memplot(log_path):
    ...
    color_vram = '#0064C8' # Cyan/Blue

    # 2. Plot RAM (Left Axis)
    ax1.set_xlabel('Steps')
    ax1.set_ylabel('RAM (GB)', color='white')
    
    line1, = ax1.plot(steps, ram_data, color=color_ram, linewidth=2.0, label='System RAM')
    
    ax1.tick_params(axis='y', labelcolor=color_ram)
    
    # Scale RAM Y-Axis: Next full integer above total, plus 10% buffer
    max_ram_val = ram_total if ram_total > 0 else (max(ram_data) if ram_data else 10)
    ylim_ram_calculated = math.ceil(max_ram_val)
    if ylim_ram_calculated == max_ram_val: ylim_ram_calculated += 1
    ylim_ram_buffered = ylim_ram_calculated * 1.1 # Add 10% buffer for headroom
    ax1.set_ylim(0, ylim_ram_buffered)
    
    # Plot RAM Limit Line
    if ram_total > 0:
        ax1.axhline(y=ram_total, color=color_ram, linestyle='--', alpha=0.5, linewidth=1.0)
        ax1.text(steps[0], ram_total, f' Total: {ram_total:.1f}GB', color=color_ram, va='bottom', fontsize=8)
    ...
    line1_d, = ax1_delta.plot(steps, delta_ram, color=color_ram, linewidth=1.5, linestyle=':', label='Process RAM Delta')
    
    # 4. Plot VRAM (Right Axis)
    ax2 = ax1.twinx()
    ax2.set_ylabel('VRAM (GB)', color='white')
    ax2.tick_params(axis='y', labelcolor=color_vram)

    # Determine Y-Limit for VRAM
    max_vram_val = vram_total if vram_total > 0 else max(vram_data) if vram_data else 0
    ylim_vram_calculated = math.ceil(max_vram_val)
    if ylim_vram_calculated == max_vram_val: ylim_vram_calculated += 1
    if ylim_vram_calculated == 0: ylim_vram_calculated = 1 # Fallback if everything is 0
    
    ylim_vram_buffered = ylim_vram_calculated * 1.1 # Add 10% buffer for headroom
    ax2.set_ylim(0, ylim_vram_buffered)

    # Handling VRAM logic (Issue 1 & 2)
    ...
    if vram_total == 0 and (not vram_data or max(vram_data) == 0):
        # Issue 1: Force only 0 tick if no VRAM
        ax2.set_ylim(0, 1) # Arbitrary low range
        ax2.set_yticks([0])
        # Issue 2: Don't plot lines
    else:
        # Normal VRAM plotting
        ax2.set_ylim(0, ylim_vram_buffered)
        
        start_vram = vram_data[0] if vram_data else 0
        delta_vram = [max(0, x - start_vram) for x in vram_data]

        line2, = ax2.plot(steps, vram_data, color=color_vram, linewidth=2.0, label='System VRAM')
        
        if vram_total > 0:
            ax2.axhline(y=vram_total, color=color_vram, linestyle='--', alpha=0.5, linewidth=1.0)
            ax2.text(steps[-1] if steps else 0, vram_total, f'Total: {vram_total:.1f}GB ', color=color_vram, va='bottom', ha='right', fontsize=8)
        ...
        line2_d, = ax2_delta.plot(steps, delta_vram, color=color_vram, linewidth=1.5, linestyle=':', label='Process VRAM Delta')
    ...
    for i, sec in enumerate(sections):
        ...
        if end_idx > start_idx:
            ...
            # Tick Generation
            # Always include 0 (local) and Duration (local)
            # Intermediate ticks: max 10 total
            ...
            for lt in local_tick_vals:
                ...
                # Offset logic for boundaries
                if lt == 0:
                    # Offset first tick slightly right to avoid hitting the divider
                    global_pos += (duration * 0.03) 
                elif lt == duration:
                    # Offset last tick slightly left
                    global_pos -= (duration * 0.03)
