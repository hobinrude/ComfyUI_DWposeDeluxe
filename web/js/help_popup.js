...
const categories = ["DWposeDeluxe"];
app.registerExtension({
	name: "DWposeDeluxe.HelpPopup",
	async beforeRegisterNodeDef(nodeType, nodeData) {
   
  if (app.ui.settings.getSettingValue("DWposeDeluxe.helpPopup") === false) {
    return;
    }
		try {
        ...
        if (nodeData?.category?.startsWith(category)) {
            if (nodeData.description) {
                let fullHtmlDescription = nodeData.description; 
                let tempDiv = document.createElement("div");
                tempDiv.innerHTML = fullHtmlDescription;
                
                let shortDescContent = "";
                const shortDescElement = tempDiv.querySelector('#dwpdlx_short_desc'); 
                if (shortDescElement) {
                    shortDescContent = shortDescElement.innerHTML.trim();
                } else {
                    let chunks = fullHtmlDescription.split('\n');
                    shortDescContent = chunks[0].trim();
                }
                
                nodeData.description = shortDescContent;
                addDocumentation(nodeData, nodeType, fullHtmlDescription); 

            } else {
                addDocumentation(nodeData, nodeType, null); 
            }
        }
        else return
    });
		} catch (error) {
			console.error("Error in registering DWposeDeluxe.HelpPopup", error);
		}
	},
});

const create_documentation_stylesheet = () => {
    const tag = 'dwpose-deluxe-documentation-stylesheet'
  ...
  export const addDocumentation = (
    ...
    nodeType.prototype.onDrawForeground = function (ctx) {
      ...
      if (this.show_doc && docElement === null) {
        ...
        docElement.classList.add('dwpose-deluxe-popup')
        
        contentWrapper.innerHTML = DOMPurify.sanitize(marked.parse(fullHtmlDescriptionContent,))
      ...
      ctx.save()
      ctx.translate(x - 2, iconSize - 51)
      ctx.scale(iconSize / 11, iconSize / 12)
      ctx.strokeStyle = 'rgb(0,0,0)'
      ctx.lineCap = 'round'
      ctx.lineJoin = 'round'
      ctx.lineWidth = 2.4
      ctx.font = this.show_doc ? '20px sans-serif' : app.canvas.title_text_font;
      ctx.fillStyle = this.show_doc ? 'rgb(255, 0, 0)' : 'rgb(153, 153, 153)';
      ctx.fillText(this.show_doc ? '×' : '?', this.show_doc ? -3 : 0, this.show_doc ? 26 : 24)
      ctx.restore()
      return r
    }
    ...
    nodeType.prototype.onMouseDown = function (e, localPos, canvas) {
      ...
      const iconX = this.size[0] - iconSize - iconMargin - 6
      const iconY = iconSize - 36
      if (
        localPos[0] > iconX &&
...
loadScript('/dwpdlx_web_async/marked.min.js').catch((e) => {
  console.log(e)
})
loadScript('/dwpdlx_web_async/purify.min.js').catch((e) => {
  console.log(e)
})
